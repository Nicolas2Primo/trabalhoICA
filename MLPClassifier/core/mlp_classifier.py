...
class AutoencoderMLP:
    def __init__(self, input_size, hidden_size, mlp_hidden_layer_sizes, learning_rate=0.01, 
                 autoencoder_epochs=100, mlp_epochs=1000, mlp_epochs_step=100, 
                 loss='mse', shuffle_data=True, early_stopping=None, momentum=0.9, heuristic=None):
        self.autoencoder = Autoencoder(input_size, hidden_size, learning_rate, autoencoder_epochs)
        self.mlp = MLP(hidden_layer_sizes=mlp_hidden_layer_sizes, learning_rate=learning_rate,
                       num_epochs=mlp_epochs, epochs_step=mlp_epochs_step, loss=loss,
                       shuffle_data=shuffle_data, early_stopping=early_stopping, momentum=momentum,
                       heuristic=heuristic)

    def train(self, X, y, X_val=None, y_val=None, plot=False):
        print("Training Autoencoder...")
        self.autoencoder.train(X, X_val)
        
        print("Encoding data...")
        X_encoded = self.autoencoder.encode(X)
        X_val_encoded = self.autoencoder.encode(X_val) if X_val is not None else None
        
        print("Training MLP...")
        self.mlp.train(X_encoded, y, X_val_encoded, y_val, plot)

    def predict(self, X):
        X_encoded = self.autoencoder.encode(X)
        return self.mlp.predict(X_encoded)